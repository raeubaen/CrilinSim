...
def main():
    ...
    E = (np.random.poisson(args.pe * E)/args.pe)/1e3
    ...
    print("E sum shape", E_sum.shape)
    
    X = np.stack([x, y, z, E], axis=-1)[E_sum > 0.5, :, :]   # (N,250,4)
    y_target = (args.be-VD_energy)[E_sum > 0.5]
    ...
    if args.train == True:
        ...
        history = model.fit(X, y_target, batch_size=64, epochs=60,validation_split=0.2, shuffle=True, callbacks=[checkpoint])
    else:
